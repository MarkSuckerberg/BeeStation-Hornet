...
/datum/species/proc/handle_mutant_bodyparts(mob/living/carbon/human/H, forced_colour)
	...
	if("teshari_head_feathers" in mutant_bodyparts)
		if(!H.dna.features["teshari_head_feathers"] || H.dna.features["teshari_head_feathers"] == "None" || H.head && (H.head.flags_inv & HIDEHAIR) || (H.wear_mask && (H.wear_mask.flags_inv & HIDEHAIR)) || !HD)
			bodyparts_to_add -= "teshari_head_feathers"

	if("teshari_eyes" in mutant_bodyparts)
		if(!H.dna.features["teshari_eyes"] || (H.wear_mask && (H.wear_mask.flags_inv & HIDEEYES)) || !HD || HD.status == BODYPART_ROBOTIC)
			bodyparts_to_add -= "teshari_eyes"
