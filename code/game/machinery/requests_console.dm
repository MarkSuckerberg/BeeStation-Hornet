	...
	if(href_list["emergency"])
		if(!emergency)
			...
			if(radio_freq)
				...
				Radio.talk_into(src,"[emergency] emergency in [department]!!",radio_freq,get_spans(),get_selected_language())
				update_icon()
...
/obj/machinery/requests_console/proc/createmessage(source, source_department, message, msgVerified, msgStamped, priority, radio_freq)
	...
	if(radio_freq)
		Radio.set_frequency(radio_freq)
		Radio.talk_into(src, "[alert]: <i>[message]</i>", radio_freq, get_spans(), get_selected_language())
